...
class Lexer():
    ...
    def __init__(self):        
        ...
        self.lineNumber = 0;               
        self.keywordList = ["abyss","absolute" , "archane", "arsenal", "attempt", "awm", "breach", "cast", "chamber", "chunk", "avatar" , "core", "dispatch", "else", "enchant", "ephemeral","enum", "expands", "false", "figure", "for", "hero", "if", "incantation","instanceof","lootbox","midget","pacify","party","persist","point","portal","powerup","plaza","save","shadow","shield", "shoot","shoots", "spawns", "stable", "supreme","synchronized","this","true","toggle ","truth","twin","unarmed","unstable","while",   "truth", "true", "false"];        
                ...
                if(self.state == State.space): 
                    if(charCurrent in operatorList):                                                
                        if(len(lexeme)>0):  #true means the end of a lexeme
                            lexemeList.append([lexeme, self.lineNumber+1, colNumber, self.getTokenBeforeAppend(lexeme)]) #add the lexeme to list          
                            lexeme = ""               #clear lexeme
